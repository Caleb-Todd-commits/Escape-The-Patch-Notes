...
interface DebugSnapshot {
  ...
  progress: Record<string, { completed: boolean; reportCollected?: boolean; bestMedal?: string; bestTime?: number }>;
}
...
test("clears every level normally and then with its replay challenge", async ({ page }) => {
  ...
  for (let level = 1; level <= totalLevels; level += 1) {
    ...
    expect(replayStart.reportVisible).toBe(true);

    const reportCollected = await page.evaluate(() => window.__patchNotesDebug!.collectReport());
    expect(reportCollected.reportCollected).toBe(true);
    ...
    expect(replayComplete.progress[level]?.reportCollected).toBe(true);
